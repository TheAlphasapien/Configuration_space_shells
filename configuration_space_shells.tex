...
\textbf{The relational structure of measurement}: Ontological plurality in our framework is fundamentally relational rather than absolute. There is no single privileged "real world"—instead, reality consists of a network of boundary points, each undergoing convergence onto locally dominant configurations. Crucially:

\begin{itemize}
\item Each boundary point experiences its own convergence event, yielding a genuine local "present moment"
\item These convergences are not causally isolated—they remain coupled through open-system dynamics ($\Gamma_{k,k'}$) and shared parameter space
\item The observed outcome at one boundary point constrains but does not determine outcomes at other boundary points
\item Consistency emerges from the geometric structure: all boundary points sample from the same thimble homology, weighted by the same Boltzmann factors
\end{itemize}

This differs fundamentally from both traditional interpretations:
\begin{itemize}
\item \textbf{vs. MWI}: No divergence into causally isolated branches. Plurality exists from the start as the spatial extent of the boundary manifold, not as temporal splitting.
\item \textbf{vs. Copenhagen}: No need for "observers" or "measurement apparatus" as primitive concepts. Convergence occurs at every boundary point as a geometric necessity.
\item \textbf{vs. Solipsism}: Your experience is real but not unique—other boundary points undergo equally real convergences. The network structure prevents collapse to a single perspective.
\end{itemize}
...
\subsection{The Relational Structure of Ontological Plurality}

Both Many Worlds Interpretation and our framework accept ontological plurality---that multiple realities exist physically, not merely as descriptions. The crucial difference lies in how this plurality is structured and how it relates to measurement.

\subsubsection{Network Structure vs. Divergent Branching}

\textbf{Traditional MWI}: Reality \textit{diverges} at each measurement. A single universe splits into causally isolated branches, proliferating exponentially with each observation. The observer ``goes down one branch,'' with other branches becoming inaccessible.

\textbf{Shell framework}: Reality is a \textit{convergent network}. Plurality exists from the outset as the spatial extent of the boundary manifold (horizon). Each boundary point undergoes geometric convergence onto dominant configurations, but these convergences remain coupled through open-system dynamics and shared parameter space.

\paragraph{Relational Nature of Measurement}
...
\paragraph{Comparison with Foundational Positions}

This relational structure differs fundamentally from all standard interpretations:

\textbf{vs. Many-Worlds Interpretation}:
\begin{itemize}
...
\textbf{vs. Copenhagen Interpretation}:
\begin{itemize}
...
\textbf{vs. Solipsism} (single observer perspective):
\begin{itemize}
...
\paragraph{The Mechanism of Plurality}
...
\textbf{MWI answer}: Through divergence. One becomes many via temporal splitting.

\textbf{Our answer}: Through convergence distribution. Many (boundary points) each select from the same menu (thimble homology), yielding consistent but not identical outcomes.
...
\paragraph{Implications for Observers}
...
This is relationalism without relativism: outcomes are relational (depend on observer location) but not arbitrary (constrained by shared geometric structure).
...
\textbf{Geometric picture}: Consider the complex amplitude $w_k = e^{-\beta E_k} e^{i\theta_k}$ associated with thimble $J_k$. In the complexified configuration space, this amplitude represents:
\begin{itemize}
\item \textbf{Forward evolution}: $e^{i\theta_k}$ phase accumulation along the thimble
\item \textbf{Dissipation}: $e^{-\beta E_k}$ suppression from coupling to other shells
\end{itemize}
...
\textbf{Physical interpretation}: As the boundary configuration evolves through complexified space, different portions of the total amplitude find new intersections. Each intersection point contributes $w_k \bar{w}_{k'}$. When $k = k'$ (self-intersection along the same thimble), the phases cancel and only the Boltzmann suppression survives—doubled because both "incoming" and "outgoing" amplitudes contribute dissipation.

\textbf{Connection to Noether's theorem}: The phase cancellation $e^{i\theta_k} e^{-i\theta_k} = 1$ reflects conservation of the quantity conjugate to the monodromy parameter. Specifically:
\begin{itemize}
\item The rotation angle $\nu$ generates monodromy with conserved angular momentum $L$
\item The thermal parameter $\beta$ conjugates to energy $E$
\item Self-intersection occurs along axes of symmetry where conserved quantities match
...
This connects to the Noether current:
\begin{equation}
J^\mu = \frac{\partial \mathcal{L}}{\partial(\partial_\mu \phi)} \delta \phi - K^\mu
\end{equation}

For the monodromy transformation $\phi \to e^{i\nu E t} \phi$, the conserved charge is precisely the energy $E$ that labels our shells. The self-intersection formula $P_k = |w_k|^2$ thus reflects the geometric requirement that probability concentrates where the symmetry-generated flow closes on itself.

\textbf{Recursive structure}: The Born rule contains a kind of self-reference: the probability of a configuration depends on how strongly that configuration selects itself through geometric convergence. Higher energy shells are suppressed not because they are "less real" but because their self-intersection strength (convergence weight) is geometrically diminished by the factor $e^{-2\beta E_k}$.

This explains why measurement "collapses" to definite outcomes: configurations with strong self-intersection (low $E_k$) naturally dominate, while high-energy configurations contribute but are geometrically suppressed. The wavefunction doesn't collapse onto these outcomes—rather, these outcomes represent the fixed points of the self-intersection dynamics in configuration space.
...
\textbf{Numerical Protocol and Expected Precision}

Figure~\ref{fig:transmon_fit} shows synthetic transmon decoherence data with measurement noise characteristic of state-of-the-art systems (5\% relative uncertainty). Linear fitting to $\Gamma = \nu|\Delta E| + \Gamma_0$ yields:

\begin{itemize}
\item Slope parameter $\nu$: recoverable to $\sim$3\% precision
\item Baseline $\Gamma_0$: larger uncertainty ($\sim$50-100\%) due to extrapolation
\item Statistical requirement: $n \geq 20$ transitions spanning factor $\sim$10 in energy
\end{itemize}
...
\paragraph{Recommended Experimental Protocol}

We propose a multi-phase measurement campaign designed to distinguish these scenarios:

\textbf{Phase 1 (Week 1): Baseline characterization}
\begin{itemize}
\item Measure all $\sim$20 transmon transitions at fixed environmental conditions
\item Repeat 3-5 times to establish baseline $\nu_0$ and statistical uncertainty
\item Expected precision: $\delta\nu_0/\nu_0 \approx 1-2\%$
\end{itemize}

\textbf{Phase 2 (Week 2): Temperature variation}
\begin{itemize}
\item Vary dilution refrigerator base temperature: $T \in [10, 50]$ mK
\item Measure full transition spectrum at each temperature
\item Shell prediction: $\nu$ constant, $\Gamma_0$ increases with $T$ (thermal occupation)
\item Standard prediction: $\nu$ increases (bath thermal noise $\propto T$)
\end{itemize}

\textbf{Phase 3 (Week 3): Bath impedance engineering}
\begin{itemize}
\item Modify readout resonator coupling via external resistor/attenuator
\item Tune engineered Purcell decay rate: $\kappa \in [0.5, 5]$ MHz
\item Shell prediction: $\nu$ unchanged, $\Gamma_0 \propto \kappa$
\item Standard prediction: $\nu \propto \sqrt{\eta} \propto \sqrt{\kappa}$ (spectral density scales)
\end{itemize}

\textbf{Phase 4 (Week 4): Electromagnetic environment}
\begin{itemize}
\item Introduce controlled broadband noise via filtered room-temperature source
\item Vary microwave pulse powers (readout and control)
\item Shell prediction: $\nu$ stable, $\Gamma_0$ increases with noise power
\item Standard prediction: Both increase (effective temperature rise)
\end{itemize}

\textbf{Analysis criteria}:
\begin{enumerate}
\item Plot $\nu$ vs. environmental parameter for all conditions
\item Calculate Pearson correlation $r(\nu, \Gamma_0)$ across all measurement runs
\item Test hypotheses:
\begin{itemize}
\item $H_{\text{shell}}$: $|r| < 0.3$ (independence) $\rightarrow$ shell structure confirmed
\item $H_{\text{std}}$: $|r| > 0.6$ (strong correlation) $\rightarrow$ standard decoherence
\end{itemize}
\item Compute variance ratio: $\sigma(\Gamma_0)/\sigma(\nu)$
\begin{itemize}
\item Shell: ratio $> 5$ (baseline varies more than slope)
\item Standard: ratio $\approx 1$ (both vary similarly)
\end{itemize}
\end{enumerate}

\paragraph{Numerical Simulation}

Figure~\ref{fig:experimental_discrimination} shows simulated results for both scenarios across 12 experimental runs with varying environmental conditions.

\textbf{Shell structure scenario} (panels a-c):
\begin{itemize}
...
\textbf{Standard decoherence scenario} (panels d-f):
\begin{itemize}
\item $\nu = 48.4 \pm 6.3$ (10$^3$ s$^{-1}$/GHz), relative variability 13\%
\item $\Gamma_0 = 9.3 \pm 2.5$ (10$^3$ s$^{-1}$), relative variability 27\%
\item Both parameters vary with bath strength
\item Note: Even with designed correlation, measurement noise can obscure it in small samples
\end{itemize}

The key discriminating feature is the \textit{differential response}: shell structure predicts $\nu$ remains stable as environmental conditions change $\Gamma_0$, while standard decoherence predicts coordinated variation. With $n \geq 10$ measurement runs spanning diverse environmental conditions, these scenarios become statistically distinguishable with $>95\%$ confidence.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{experimental_discrimination.png}
\caption{\textbf{Experimental discrimination between shell structure and standard decoherence.} Simulated transmon measurements across 12 runs with varying environmental conditions. \textbf{Top row (a-c)}: Shell structure scenario showing stable $\nu$ (3.8\% variation) despite significant $\Gamma_0$ fluctuations (30\%). No correlation between parameters ($r = -0.24$, $p = 0.45$). \textbf{Bottom row (d-f)}: Standard Ohmic bath scenario where both $\nu$ and $\Gamma_0$ respond to bath variations. The differential stability provides a clear experimental signature: geometric coupling yields parameter independence, while environmental coupling produces coordinated variation.}
\label{fig:experimental_discrimination}
\end{figure}
